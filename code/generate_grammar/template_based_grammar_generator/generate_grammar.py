...
def find_dep_tree_correspondences(tree, dep):
    ...
    for current_dep in dep:
        ...
        template_name = ""
        if ancestor_info["arity"] == 2: # unaries have to be solved elsewhere
        ...
        if ancestor_info["is_reverse"] == True:
            rtg_type = "{} -> ({}, {})".format(ancestor_info["common_ancestor"].label(), ancestor_info["child2"].label(), ancestor_info["child1"].label())
        else:
            rtg_type = "{} -> ({}, {})".format(ancestor_info["common_ancestor"].label(), ancestor_info["child1"].label(), ancestor_info["child2"].label())
...
def main(fn1, fn2):
    ...
    with open(fn1) as tree_file, open(fn2) as dep_file:
        while True:
            ...
            find_dep_tree_correspondences(tree, dep)
